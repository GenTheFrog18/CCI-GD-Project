# Spesifikasi Item untuk Artist, Programmer, dan Sound Designer

## Tujuan Dokumen

Dokumen ini menjelaskan semua item yang direncanakan untuk build game jam. Dokumen ini merupakan file kerja mandiri untuk **artist, programmer, dan sound designer**. Setiap spesifikasi menjelaskan pengalaman pemain, behavior, visual, audio, dan kebutuhan teknis agar ketiga peran dapat bekerja dari referensi yang sama tanpa menebak aturan dasarnya.
...
## Cara Menggunakan Dokumen Berdasarkan Peran

### Untuk artist

- Gunakan bagian deskripsi belum dikenal untuk menentukan petunjuk visual pertama.
- Gunakan deskripsi sudah dikenal dan behavior untuk memastikan bentuk item menjelaskan kegunaannya.
- Buat icon inventory, world sprite, state visual, animation, dan VFX yang tercantum.
- Jangan menentukan collision atau ukuran gameplay sendiri; konfirmasikan dengan programmer.
- Prioritaskan silhouette dan state penting sebelum variasi dekoratif.

### Untuk programmer

- Gunakan ID saran sebagai ID stabil setelah disetujui tim.
- Implementasikan item melalui `ItemDefinition`, behavior item, dan `ThrownItem` bersama.
- Simpan hanya state yang tercantum sebagai persisten.
- Hubungkan animation, VFX, dan audio melalui signal/hook agar asset dapat diganti tanpa menulis ulang behavior.
- Jaga semua nilai playtest di Resource atau Inspector.

### Untuk sound designer

- Gunakan behavior dan risiko item untuk menentukan fungsi setiap cue, bukan hanya tema visualnya.
- Pisahkan suara pickup, use, throw, impact, active loop, warning, break, dan expire bila relevan.
- Berikan nama file/event yang stabil dan konsisten dengan ID item.
- Sediakan variasi hanya setelah satu cue utama setiap event sudah tersedia.
- Pastikan suara penting untuk gameplay—seperti Rattlepod, Lantern Snail, dan Silver Weight—mudah dibedakan dari ambience dan musuh.

### Urutan produksi asset

1. Icon dan world sprite placeholder/final dasar.
2. Satu cue audio utama untuk use/impact yang paling penting.
3. State visual mekanis seperti aktif, kosong, retak, atau hampir habis.
4. Animation dan VFX tambahan.
5. Variasi audio dan polish.
...
# 7. Matriks Deliverable per Peran

| Item | Artist | Sound Designer | Programmer |
| --- | --- | --- | --- |
| Rope | Icon gulungan, world coil, rope tile/line, anchor | Pasang, tali menegang, panjat | Placement, validasi, climbing, persistence |
| Red Whistle | Icon dan sprite merah | Cue penggantian/kehilangan opsional | Slot khusus, pencurian bersyarat, replacement |
| Blue Whistle | Icon dan sprite biru, efek award | Cue progres Blue Whistle | Delivery threshold, replace whistle, gate check |
| Moon Whistle | Icon/sprite final paling istimewa, efek ending | Cue award dan ending | Final gate reward, save, ending trigger |
| Multitool | Sprite, animasi swing, impact VFX | Swing, batu pecah, hit makhluk | Interaction priority, break, harvest, weak damage |
| Bandage | Icon, world pack, healing feedback | Kain, use, healing tick/finish | Stop bleed, heal-over-time, modifier healing |
| Info Book | Icon/world book, UI unlock effect | Buka buku, knowledge unlock | Unlock-all meta knowledge, UI refresh |
| Numbing Pill | Icon obat, status icon | Consume, suppression start/end | Timer 300 detik, curse suppression, save |
| Sun Sphere | State inactive/active/fading, warm light | Activate, throw, fade | Light timer, throw impact, cleanup |
| Throwable Rock | World/icon rock, spin sederhana | Throw, terrain hit, creature hit | Velocity damage, knockback, retrieval |
| Lantern Snail | Calm/agitated/scream/cooldown, shell light | Teriakan khas, agitation, impact | Harvest, light, sound event, dazzle, lure |
| Rattlepod | Full/empty state, shake animation | Pulsa rattle yang sangat terbaca | Sound event pulses, charges, throw behavior |
| Hushcap | Mushroom sprite, cloud VFX | Crush, spore burst, cloud ambience opsional | Sight-obscuring area, duration, cleanup |
| Cling Resin | Tree source, container, sticky patch | Pour, splash, sticky movement | Slow area, body tracking, projectile modifier |
| Driftseed | Seed, attachment state, upward particles | Attach, airy loop, detach | Gravity/knockback status, target whitelist, timer |
| Silver Weight | Intact/cracked states, heavy impact, break VFX | Heavy throw, impact, crack, shatter | Durability, small-enemy kill, persistence, sale state |

---

# 8. Keputusan Desain yang Masih Perlu Dikunci
